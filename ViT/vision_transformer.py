...
class Block(nn.Module):
    ...
    def forward(self, x, layer_index, start_time, budget, oracle, last_layer_consume_time):
        
        x = x + self.drop_path(self.attn(self.norm1(x)))
        decision = oracle.make_drop(layer_index, start_time , budget, last_layer_consume_time, self.attn.attn)
        
        if decision == None :
            '''
                the budget has already satisfied
            '''
            pass
            ...
            '''
                we should follow the decision made by the oracle
            '''
            x = x[:,decision,:].contiguous()
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        '''
            here we add the logics of bugetting
        '''
