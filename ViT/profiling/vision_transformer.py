...
class Block(nn.Module):
    ...
    def forward(self, x, attention_time_list, MLP_time_list):
        start_time = time.time()
        x = x + self.drop_path(self.attn(self.norm1(x)))
        attention_time = time.time()
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        mlp_time = time.time()
        attention_time_list.append(attention_time - start_time)
        MLP_time_list.append(mlp_time - attention_time)
        self.attn_score = self.attn.attn
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            x = blk(x,self.attention_time_list, self.MLP_time_list)
        # print('blocks time=', time.time()-block_start)
        norm_time_start = time.time()
        x = self.norm(x)
        # print('norm time=',time.time()-norm_time_start)
        if self.dist_token is None:
    ...
    def forward(self, x):
        ...
        last_part_time_begin = time.time()
        ...
        self.endphase_time = time.time() - last_part_time_begin
